...
___console_logs '[12/22] Copy the certificate and key'
...
___console_logs '[15/22] Create Dockerfile'
...
___console_logs '[16/22] Build NGINX docker image'
docker build -t nginx-minikube-proxy -f $NGINX_FOLDER/Dockerfile $MINIKUBE_INSTALL_ROOT_FOLDER

___console_logs '[17/22] Run NGINX docker image'
...
___console_logs '[18/22] Configure Kubeconfig to external access'
...
___console_logs '[19/22] Show all Configuration Files'
...
___console_logs '[20/22] Create Kubernetes Dashboard Ingress'
...
___console_logs '[21/22] Configure iptable'
